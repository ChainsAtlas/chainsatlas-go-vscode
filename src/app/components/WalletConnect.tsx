...
const Test = (): React.ReactElement => {
  ...
  //   onConnect: ({ connector }) => {
  //     if (connector) {
  //       connector.addListener("connect", (data) => {
  //         console.log("conectei fdp", data);
  //       });

  //       connector.addListener("error", (error) => {
  //         console.log("namoreds?:", error);
  //       });
  //     }
  //   },
  // });
...
const WalletConnect = (): React.ReactElement => {
  ...
  //     w3mProvider({ projectId: "7b1ecd906a131e3a323a225589f75287" }),
  //   ]);
  ...
  React.useEffect(() => {
    window.addEventListener("message", (event) => {
      const message: { command: string; value: string } = event.data;
    ...
    <div>
      {uri && (
        <QRCodeSVG
          value={uri}
          imageSettings={{
            src: "https://chainsatlas.com/wp-content/uploads/2022/08/ChainsAtlas-logo.png",
            height: 250,
            width: 250,
            excavate: true,
          }}

...
import { FMT_BYTES, FMT_NUMBER, type ContractAbi, type Web3 } from "web3";

class VirtualizationUnit extends EventEmitter {
  private static readonly _ABI: ContractAbi = [
    {
      anonymous: false,
      inputs: [
        {
  ...
  private static readonly _BYTECODE =
  ...
  public async deploy(from: string, web3: Web3): Promise<void> {
    try {
      ...
      const contract = new web3.eth.Contract(VirtualizationUnit._ABI);

      const deployment = contract.deploy({
        data: VirtualizationUnit._BYTECODE,
      });
